...
def get_podcast_guest(podcast_transcript):
    ...
    for guest in guests:
        wiki = None
        try:
            wiki = wikipedia.page(f"{guest.get('name')} {guest.get('title')} {guest.get('organization')}", auto_suggest=True)
        except:
            None
...
def process_podcast(url, path):
    ...
    podcast_guests = get_podcast_guest.call(podcast_details['episode_transcript'])
